...
class NodeWSTunnel extends EventEmitter
    constructor:(ws)->
        ...
        ws.on "error",(err)=>
            console.log "tunnel-error",err,ws._socket && ws._socket.address or "no socket","?"
            @isReady = false
        ws.on "close",()=>
            @emit "close"
            @isReady =false
...
class RPCInterface extends EventEmitter
    ...
    deferBuffer:()->
        console.log "defer buffer"
        for item in @callBuffer
            console.log "defername:",item.name
            @[item.name].apply(this,item.args)
...
class RPCServer extends EventEmitter
    ...
    setTunnel:(tunnel)->
        ...
        @tunnel.on "close",(data)=>
            ...
            @isReady = false
    declare:(name)->
        # We actully parse args form Function.length
